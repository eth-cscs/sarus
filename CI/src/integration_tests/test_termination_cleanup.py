...
class TestCleanupAfterSignals(unittest.TestCase):
    ...
    process, which then terminates and allows the OCI runtime to cleanup and
    ...
    The tests tearDown() methods attempt to cleanup if Sarus is unsuccessful in
    doing so by itself.
    """
    ...
    def tearDown(self):
        ...
        # We cleanup the children in reversed order, thus terminating the
        # container processes first, leaving the OCI Runtime last to give
        # it the chance to cleanup the cgroup subdir it created.
    ...
    def _run_test(self, run_options, commands, sig):
        sarus_process = psutil.Popen(["sarus", "run"] + run_options + [self.__class__.container_image] + commands)
        time.sleep(2)
