...
TEST(MPIHookTestGroup, test_mpi_libraries_injection) {
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries(
        {"/usr/local/lib/libmpi.so.12.5.5","/opt/mpi_hook/libmpi.so.12","/opt/mpi_hook/libmpi.so"})
      .checkSuccessful();

   // multiple host and container libraries, one version of each
  Checker{}
      .setHostMpiLibraries({"/lib/libmpi.so.12.5.5",
                            "/lib/libmpicxx.so.12.5.5"})
      .setPreHookContainerLibraries(
          {"/lib/libmpi.so.12.5.5", "/lib/libmpicxx.so.12.5.5"})
      .expectPostHookContainerLibraries(
          {"/opt/mpi_hook/libmpi.so", "/opt/mpi_hook/libmpi.so.12", "/lib/libmpi.so.12.5.5",
           "/opt/mpi_hook/libmpicxx.so", "/opt/mpi_hook/libmpicxx.so.12", "/lib/libmpicxx.so.12.5.5"})
      .checkSuccessful();
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries(
          {"/opt/mpi_hook/libmpi.so", "/opt/mpi_hook/libmpi.so.12",
           "/lib/libmpi.so.12.5.5",

           "/opt/mpi_hook/libmpicxx.so", "/opt/mpi_hook/libmpicxx.so.12",
           "/opt/mpi_hook/libmpicxx.so.12.5.5"})
      .checkSuccessful();
}
...
TEST(MPIHookTestGroup, test_dependency_libraries_injection) {
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({"/lib/libmpi.so.12", 
                                         "/opt/mpi_hook/libmpi.so",
                                         "/opt/mpi_hook/libdep.so", 
                                         })
      .checkSuccessful();
  ...
  Checker{}
      .setHostMpiLibraries({"/lib/libmpi.so.12"})
      .setHostMpiDependencyLibraries({"/lib/libdep.so"})
      .setPreHookContainerLibraries({"/lib/libmpi.so.12", "/lib/libdep.so"})
      .expectPostHookContainerLibraries({"/lib/libmpi.so.12", "/lib/libdep.so", "/opt/mpi_hook/libmpi.so" })
      .checkSuccessful();
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12", 
        "/opt/mpi_hook/libmpi.so", 
        "/opt/mpi_hook/libdep0.so", "/opt/mpi_hook/libdep1.so"
       })
        .checkSuccessful();
...
TEST(MPIHookTestGroup, test_libraries_injection_container_version_matching) {
  ...
  */
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12.1", "/lib/libmpi.so.12.2",
        "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so"
        })
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12.1.1", "/lib/libmpi.so.12.5", "/lib/libmpi.so.12.2.2",
        "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so"
      })
        .expectPreservedPostHookContainerLibraries({
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12.1", "/lib64/libmpi.so.13.1",
        "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so"
      })
      .checkFailure();
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12.1", "/lib/libdep.so.4.6", "/lib/libdep.so.4.4",
        "/lib/libdep.so.4.3", "/lib64/libdep.so.4.2",
        "/lib64/libdep.so.5.1",
        "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so",
        "/opt/mpi_hook/libdep.so.4", "/opt/mpi_hook/libdep.so"
      })
      .expectPreservedPostHookContainerLibraries({
          "/lib/libdep.so.4.6",
          "/lib/libdep.so.4.3",
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
            "/lib/libmpi.so.12.1", "/lib64/libdep.so.5.0", "/lib/libdep.so.4.3",
            "/lib/libdep.so.4.5",
            "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so",
            "/opt/mpi_hook/libdep.so.4", "/opt/mpi_hook/libdep.so"
      })
  ...
  // 2 newer
    // the implementation assumes that only the major number is relevant to
    // decide whenter to replace or add a new libray. Compare to the previous version,
    // both /lib64/libdep.so.4.3 and /lib64/libdep.so.4.5 are ABI compatible with 
    // /lib/libdep.so.4.2. The hook hence mounts /lib/libdep.so.4.2 -> /lib64/libdep.so.4.3
    // (being the closer subversion)
  Checker{}
      .setHostMpiLibraries({"/lib/libmpi.so.12.1"})
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12.1", "/lib64/libdep.so.4.3", "/lib64/libdep.so.4.5",
                    "/opt/mpi_hook/libmpi.so.12", "/opt/mpi_hook/libmpi.so",
            "/opt/mpi_hook/libdep.so.4", "/opt/mpi_hook/libdep.so"
      })
      .expectPreservedPostHookContainerLibraries({
...
TEST(MPIHookTestGroup, test_dependency_libraries_injection_container_version_matching) {
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
        "/lib/libmpi.so.12", "/lib64/libdep.so.3", "/lib64/libdep.so.4",
        "/opt/mpi_hook/libmpi.so",
        "/opt/mpi_hook/libdep.so"
      })
      .expectPreservedPostHookContainerLibraries({
  ...
  Checker{}
      ...
      .expectPostHookContainerLibraries({
          "/lib/libmpi.so.12", "/lib64/libdep.so.4", "/lib64/libdep.so.5",
          "/opt/mpi_hook/libmpi.so",
          "/opt/mpi_hook/libdep.so"
      })
      .expectPreservedPostHookContainerLibraries({
...
TEST(MPIHookTestGroup, test_mpi_libraries_injection_mounted_on_annotated_path) {
  // MPI library in non-default linker directory
  Checker{}
      .setCustomMountParentDir("/opt/xxx")
      .setHostMpiLibraries({"/lib/aarch64-linux-gnu/libmpi.so.12.5.5"})
      .setPreHookContainerLibraries({"/usr/local/lib/libmpi.so.12.5.5"})
      .expectPostHookContainerLibraries(
        {"/usr/local/lib/libmpi.so.12.5.5","/opt/xxx/mpi_hook/libmpi.so.12","/opt/xxx/mpi_hook/libmpi.so"})
      .checkSuccessful();
  ...
  Checker{}
      ...
      .setPreHookContainerLibraries(
          {"/lib/libmpi.so.12.5.5", "/lib/libmpicxx.so.12.5.5"})
      .expectPostHookContainerLibraries(
          {"/opt/xxx/mpi_hook/libmpi.so", "/opt/xxx/mpi_hook/libmpi.so.12", "/lib/libmpi.so.12.5.5",
           "/opt/xxx/mpi_hook/libmpicxx.so", "/opt/xxx/mpi_hook/libmpicxx.so.12", "/lib/libmpicxx.so.12.5.5"})
      .checkSuccessful();

  // multiple libraries (not all in container, but all injected)
  // Note: we inject all the host MPI libraries also when they are not present
  // in the container because we don't know about the dependencies between the
  // host's MPI libraries. E.g. libmpicxx.so might depend on libmpi.so
  Checker{}
      .setCustomMountParentDir("/opt/xxx")
      .setHostMpiLibraries(
          {"/lib/libmpi.so.12.5.5", "/lib/libmpicxx.so.12.5.5"})
      .setPreHookContainerLibraries({"/lib/libmpi.so.12.5.5"})
      .expectPostHookContainerLibraries(
          {"/opt/xxx/mpi_hook/libmpi.so", "/opt/xxx/mpi_hook/libmpi.so.12",
           "/lib/libmpi.so.12.5.5",

           "/opt/xxx/mpi_hook/libmpicxx.so", "/opt/xxx/mpi_hook/libmpicxx.so.12",
           "/opt/xxx/mpi_hook/libmpicxx.so.12.5.5"})
      .checkSuccessful();
