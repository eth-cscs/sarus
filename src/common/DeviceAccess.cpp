...
void DeviceAccess::parseInput(const std::string& input) {
    ...
    logMessage(boost::format("Correctly parsed device access permissions: %s") % string(), common::LogLevel::DEBUG);
}
...
void DeviceAccess::logMessage(const boost::format& message, common::LogLevel level,
                std::ostream& out, std::ostream& err) const {
    logMessage(message.str(), level, out, err);
}

void DeviceAccess::logMessage(const std::string& message, common::LogLevel level,
                std::ostream& out, std::ostream& err) const {
